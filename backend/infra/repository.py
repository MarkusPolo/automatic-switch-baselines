...
def update_run_device_status(db: Session, run_id: int, device_id: int, status: str, error_message: Optional[str] = None, error_code: Optional[str] = None, template_hash: Optional[str] = None, tasks: Optional[str] = None, captured_config: Optional[str] = None):
    ...
    if status == "RUNNING":
        ...
        if tasks:
            db_rd.tasks = tasks
        if captured_config:
            db_rd.captured_config = captured_config
