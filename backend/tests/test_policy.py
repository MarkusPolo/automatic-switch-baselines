...
@pytest.mark.asyncio
async def test_validate_hostname_valid():
    device = models.Device(
        id=1, job_id=1, hostname="sw-lab-01", 
        mgmt_ip="10.0.0.1", mask="255.255.255.0", gateway="10.0.0.254"
    )
    errors = await policy.validate_device_config(device, [device])
    assert len(errors) == 0
...
@pytest.mark.asyncio
async def test_validate_subnet_mismatch():
    device = models.Device(
...
@pytest.mark.asyncio
async def test_validate_duplicate_ip():
    ...
    errors = await policy.validate_device_config(d2, [d1, d2])
    assert any(e.field == "mgmt_ip" and "Duplicate" in e.message for e in errors)

@pytest.mark.asyncio
async def test_validate_duplicate_port():
...
@pytest.mark.asyncio
async def test_validate_vlan_range():
    device = models.Device(
